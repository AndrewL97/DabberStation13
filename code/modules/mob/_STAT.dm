...
/mob/Stat() //human.dm line 147
	...
	if(statpanel("Debugging information")) //Debug.
		...
		if(master_controller)
			...
			stat(null, "Max actions : [max_actions - ((max(0,min(world.cpu,100))/100)*(max_actions/2)*(CPU_warning))][CPU_warning ? " (THROTTLED)" : ""]")
		if(air_master)
			stat("ATMOS",null)
		...
		if(water_master)
			...
			stat(null, "Max actions : [max_actions_water - ((max(0,min(world.cpu,50))/50)*(max_actions_water/1.5))]")