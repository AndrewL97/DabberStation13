...
var
	...
	sd_dark_shades = 255					// number of icon state in sd_dark_icon
...
area
	...
	proc
		sd_LightLevel(slevel = sd_light_level as num, keep = 1, turf = null)
			if(!src) return
			overlays -= sd_darkimage

			if(keep) sd_light_level = slevel

			slevel = min(max(slevel,0),255)	// restrict range
			...
			var/image/light = image(sd_dark_icon,,"RGB",sd_light_layer)
			//reee.alpha = 255-(slevel*25)
			//255-
			var/C1 = max(255-(slevel*25),0)
			/*
