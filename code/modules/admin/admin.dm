...
//		var/list/alljobs = get_all_jobs()
		...
		usr << browse(cssStyleSheetKaden + dat, "window=jobban2;size=600x150")
		return
	...
	if (href_list["removejobban"])
		if ((src.rank in list("Dab13 Administrator", "Host"  )))
	...
	if (href_list["sendtoprison"])
		if ((src.rank in list( "Moderator", "Administrator", "Secondary Administrator", "Primary Administrator", "Shit Guy", "Dab13 Administrator", "Host"  )))
	...
	if (href_list["makeai"]) //Yes, im fucking lazy, so what? it works ... hopefully
		if ((src.rank in list( "Primary Administrator", "Dab13 Administrator", "Host", "Administrator", "Shit Guy"  )))
...
/obj/admins/proc/announce()
	...
	if (message)
		if(usr.client.holder.rank != "Dab13 Administrator" && usr.client.holder.rank != "Host")
...
/obj/admins/proc/toggleDabbersay()
	set category = "Special Verbs"
	set desc = "Toggle dis bitch"
	set name = "Toggle Dabbersay"
	Dabbersay_allowed = !( Dabbersay_allowed )
	if (Dabbersay_allowed)
		world << "<B>The DabberSAY channel has been enabled.</B>"
	else
		world << "<B>The DabberSAY channel has been disabled.</B>"
	log_admin("[key_name(usr)] toggled Dabbersay to [Dabbersay_allowed].")
	message_admins("[key_name_admin(usr)] toggled DabberSAY [Dabbersay_allowed ? "on" : "off"]", 1)
...
/client/New()
	..()
	if(key == world.host || key == "AlcaroIsAFrick" || key == "Riki2321gamer") //also have a backup here incase some shit happens and world.host isn't me (incase im hosting on a vps or some shit)
		update_admins("Host")
