...
/obj/item/weapon/gun/energy/pulse_rifle
	...
	afterattack(atom/target as mob|obj|turf|area, mob/user as mob, flag)
		...
		if(mode == 1)
			if (targloc == curloc)
				if(user.can_bullet_act()) user.bullet_act(PROJECTILE_PULSE)
				return
		else if(mode == 2)
			if (targloc == curloc)
				if(user.can_bullet_act()) user.bullet_act(PROJECTILE_TASER)
				return
...
/obj/item/weapon/gun/revolver/afterattack(atom/target as mob|obj|turf|area, mob/user as mob, flag)
	...
	if (!( istype(T, /turf) ))
		return
	if (U == T)
		if(user.can_bullet_act()) user.bullet_act(PROJECTILE_BULLET)
		return
...
/obj/item/weapon/gun/detectiverevolver/afterattack(atom/target as mob|obj|turf|area, mob/user as mob, flag)
	...
	if (!( istype(T, /turf) ))
		return
	if (U == T)
		if(user.can_bullet_act()) user.bullet_act(PROJECTILE_WEAKBULLET)
		return
...
/obj/item/weapon/gun/energy/laser_gun/afterattack(atom/target as mob|obj|turf|area, mob/user as mob, flag)
	...
	if(!istype(T, /turf))
		return
	if(U == T)
		if(user.can_bullet_act()) user.bullet_act(PROJECTILE_LASER)
		return
	if(!istype(U, /turf))
		return
...
/obj/item/weapon/gun/energy/crossbow/afterattack(atom/target as mob|obj|turf|area, mob/user as mob, flag)
	...
	if(!istype(T, /turf))
		return
	if (U == T)
		if(user.can_bullet_act()) user.bullet_act(PROJECTILE_BOLT)
		return
	if(!istype(U, /turf))
		return
