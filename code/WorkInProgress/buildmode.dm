	...
	if(pa.Find("left"))
		...
		switch(master.cl.buildmode)
			...
			if(2)
				...
				else if (dd_hasprefix(objholder, "/mob") && !(usr.client.holder.rank in list("Host", "Dab13 Administrator", "Shit Guy")))
					objholder = "/obj/closet"
			if(3)
				var/list/locked = list("vars", "key", "ckey", "client", "firemut", "ishulk", "telekinesis", "xray", "virus", "cuffed", "ka", "last_eaten", "urine")

				master.buildmode.varholder = input(usr,"Enter variable name:" ,"Name", "name")
				if(master.buildmode.varholder in locked && !(usr.client.holder.rank in list("Host", "Dab13 Administrator")))
					return
	...
	switch(buildmode)
		...
		if(1)
			if(istype(object,/turf) && pa.Find("left") && !pa.Find("alt") && !pa.Find("ctrl") )
				...
				new/obj/window_spawner(get_turf(object))
		if(2)
			if(pa.Find("left"))
